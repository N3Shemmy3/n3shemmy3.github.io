<template>
  <div id="projects-title" class="mt-24 content-center">
    ...
    <div class="mx-auto grid gap-4 grid-responsive py-4">
      ...
      <ProjectSkeleton v-else v-for="n in 3" :key="n" />
    </div>
  </div>
</template>
...
onMounted(() => {
  getUserReposWithAuth(username, accessToken).then((repos) => {
    if (repos) {
...
async function getUserReposWithAuth(
  username: string,
  accessToken: string
): Promise<any | null> {
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      const warning = `Rate limit exceeded. Please wait for ${
        timeUntilReset / 1000
